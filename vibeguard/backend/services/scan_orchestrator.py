...
Orchestrates a full VibeGuard scan:
    ...
    5. Ask Gemini to explain each finding (best-effort, non-fatal)
...
MAX_GEMINI_FINDINGS = 25  # cap explained findings to keep demo scans fast/affordable
...
def run_full_scan(
    ...
    report = models.Report(
        ...
        gemini_available="unknown",
    )
    db.add(report)
    db.flush()  # obtain report.id

    # --- Gemini explanations (best-effort, non-fatal) -----------------------------
    gemini_used = False
    gemini_attempted = False
    if use_gemini and gemini_service.is_configured():
        gemini_attempted = True
        for finding in all_findings[:MAX_GEMINI_FINDINGS]:
            explanation = gemini_service.explain_finding(
        ...
        # Any remaining findings beyond the Gemini cap are still stored, just
    ...
    report.gemini_available = (
        "unavailable" if not gemini_attempted else ("available" if gemini_used else "unavailable")
    )
