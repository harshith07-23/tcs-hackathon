...
VibeGuard scans a project (Python / JavaScript / TypeScript) for security issues using deterministic static analysis, secret detection, and dependency checks — then uses **Groq** to explain findings, describe potential attack paths, and suggest fixes. The numeric security score is always deterministic and never depends on Groq.
...
Deterministic Security Analysis  (semgrep / bandit / built-in pattern rules)
   ...
   Dependency Analysis (requirements.txt / package.json vs known advisories)
        ...
        Risk Scoring Engine (0-100, fully reproducible)
              +
        Groq Reasoning Layer (explanation, attack path, remediation)
              =
           VibeGuard Report  →  PostgreSQL
```

Groq is never the sole source of truth — if it is unavailable, misconfigured, or returns malformed output, the scan still completes and returns the deterministic findings.
...
│   │   ├── groq_service.py     Groq explanation layer
...
- A Groq API key from [Groq Console](https://console.groq.com) (optional — the scanner works without it, just without AI explanations)
...
Replace `YOUR_PASSWORD` with the PostgreSQL password you set in step 3, and `YOUR_GROQ_API_KEY` with your Groq key. **Never commit this file** — it's already listed in `.gitignore`.
...
This installs FastAPI, SQLAlchemy, `psycopg2-binary`, `python-dotenv`, the `groq` SDK, and the security scanning dependencies (`semgrep`, `bandit`). VibeGuard's built-in pattern-based rules and secret patterns work even if `semgrep`/`bandit`/`gitleaks` are not installed or fail to run — they are optional enhancements, not hard requirements.
...
If `GROQ_API_KEY` is not set, `ml_model` will show `"unavailable"` but the rest of the API still works.
...
| `GET` | `/api/health` | Database + Groq availability status |

---

## 14. Security notes

- Secrets are **masked** before storage/display (e.g. `AKIA************9XYZ`) — full values are never persisted or sent to Groq.
...
- Database and Groq credentials are read only from environment variables and are never logged or returned in API error messages.
- The risk score is a **pure, reproducible function** of deterministic findings — Groq never influences it.
...
**`ml_model: unavailable` at `/api/health`** — `GROQ_API_KEY` is missing or invalid. The scanner still works; you'll just see deterministic findings without AI explanations.
